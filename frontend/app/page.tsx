...
export default function KirinyagaSouthSuperLeague() {
  ...
  const [fixtureForm, setFixtureForm] = useState({ homeTeamId: '', awayTeamId: '', date: '', venue: '', gameweek: 11, stage: 'Regular Season' });
  ...
  const openFixtureModal = (fixture: Match | null = null) => {
    if (fixture) {
      ...
      setFixtureForm({
        ...
        stage: fixture.stage || 'Regular Season'
      });
    } else {
      setEditingFixture(null);
      setFixtureForm({ homeTeamId: '', awayTeamId: '', date: '', venue: '', gameweek: 11, stage: 'Regular Season' });
    }
  ...
  // Helper for consistent match card rendering
    ...
    return (
      <div key={match.id} className="bg-slate-950 rounded-xl p-4 border border-slate-800 space-y-3 relative">
        ...
        <div className="text-xs text-slate-400 font-semibold bg-emerald-950/60 inline-block px-2 py-1 rounded text-emerald-400">
          {match.stage && match.stage !== 'Regular Season' ? match.stage : `GW ${match.gameweek}`} • {match.date}
        </div>
          ...
          <main className="lg:col-span-9 space-y-8">
                  ...
                  <table className="w-full text-left text-sm text-slate-300">
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {sortedTeams.map((team, idx) => (
                        <tr key={team.id} className={`transition hover:bg-slate-800/40 ${idx === 13 ? 'bg-rose-900/40' : ''}`}>
                          <td className="py-3 px-4 text-center font-bold relative">
                            <div className="flex items-center justify-center">
                              {idx === 0 && <Trophy className="w-4 h-4 text-amber-400 absolute left-2" />}
                              <span>{idx + 1}</span>
                            </div>
                          </td>
            ...
            {activeSection === 'matches' && (
              <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl p-6">
                <h3 className="text-xl font-extrabold text-white flex items-center space-x-2 border-b border-slate-800 pb-4 mb-6">
                  <Flame className="w-5 h-5 text-amber-400" /><span>Regular Season Fixtures</span>
                </h3>
                
                {/* Regular Matches */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {matches.filter(m => !m.stage || m.stage === 'Regular Season').map(renderMatchCard)}
                </div>

                {/* Championship Stages Sections */}
                <div className="mt-12 space-y-8 border-t border-slate-800 pt-8">
                  <h3 className="text-xl font-extrabold text-white flex items-center space-x-2 pb-2">
                    <Trophy className="w-6 h-6 text-amber-400" /><span>Championship Playoffs</span>
                  </h3>

                  {[
                    { key: 'QF Playoffs', title: 'Quarter Finals Play Offs (4 Fixtures)' },
                    { key: 'QF Knockouts', title: 'Quarter Finals Knockouts (4 Fixtures)' },
                    { key: 'Semifinals', title: 'Semi-Finals (2 Fixtures)' },
                    { key: 'Third Place', title: '3rd & 4th Place Playoff (1 Fixture)' },
                    { key: 'Finals', title: 'Grand Finale (1 Fixture)' }
                  ].map(stage => {
                    const stageMatches = matches.filter(m => m.stage === stage.key);
                    return (
                      <div key={stage.key} className="space-y-3">
                        <h4 className="text-sm font-bold text-emerald-400 uppercase tracking-wider bg-slate-800/40 inline-block px-3 py-1 rounded">
                          {stage.title}
                        </h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {stageMatches.map(renderMatchCard)}
                          {stageMatches.length === 0 && (
                             <div className="col-span-full text-slate-500 italic text-sm py-6 border border-slate-800/50 border-dashed rounded-xl flex justify-center items-center">
                               Waiting for playoff fixtures to be drawn...
                             </div>
                          )}
                        </div>
                      </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input 
                    ...
                    name="admin_login_pass_no_autofill"
                    value={adminPassword} 
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 relative">
            ...
            <form onSubmit={handleSaveFixture} className="space-y-4">
              <select required value={fixtureForm.stage} onChange={e => setFixtureForm({...fixtureForm, stage: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-sm text-white">
                <option value="Regular Season">Regular Season</option>
                <option value="QF Playoffs">Quarter Finals Play Offs</option>
                <option value="QF Knockouts">Quarter Finals Knockouts</option>
                <option value="Semifinals">Semi-Finals</option>
                <option value="Third Place">3rd & 4th Place Playoff</option>
              ...
              <input type="number" placeholder="Gameweek (If Regular Season)" value={fixtureForm.gameweek} onChange={e => setFixtureForm({...fixtureForm, gameweek: Number(e.target.value)})} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-sm text-white" />
...
const BACKEND_URL = '[https://kirinyaga-south-super-league-6.onrender.com](https://kirinyaga-south-super-league-6.onrender.com)';
...
export default function KirinyagaSouthSuperLeague() {
  ...
  // Positions dynamically recalculate on every state update
  const sortedTeams = useMemo(() => {
    return [...teams].sort((a, b) => {
      const gdA = a.gf - a.ga;
      const gdB = b.gf - b.ga;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col antialiased selection:bg-emerald-500 selection:text-black">
      ...
      <section className="relative w-full h-[320px] sm:h-[420px] overflow-hidden bg-slate-900 border-b border-emerald-950">
        {combinedHeroMedia.map((m, idx) => (
          <div key={m.id + idx} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${idx === currentSlide ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
            <MediaRenderer className="w-full h-full filter brightness-[0.35] scale-105 transform transition-transform duration-[5000ms]" url="{m.url}"/>
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent"></div>
          </div>
        ))}
              ...
              <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm text-slate-300">
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {sortedTeams.map((team, idx) => (
                        <tr key={team.id} className={`transition hover:bg-slate-800/40 ${idx === 13 ? 'bg-[#800000]/40' : ''}`}>
                          ...
                          <td className="py-3 px-4 font-semibold text-white flex items-center space-x-3">
                            <MediaRenderer className="w-8 h-8 rounded-full border border-slate-700" url="{team.logo}"/>
                            <div>
                              <p>{team.name}</p>
                              <p className="text-xs text-slate-500 font-normal">{team.town}</p>
                            </div>
            ...
            {activeSection === 'matches' && (
              <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl p-6">
                  ...
                  {matches.map((match) => {
                    ...
                    return (
                      <div key={match.id} className="bg-slate-950 rounded-xl p-4 border border-slate-800 space-y-3 relative">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="text-center flex-1">
                            <MediaRenderer ''} className="w-10 h-10 mx-auto rounded-full" url="{homeTeam?.logo" ||/>
                            <p className="text-sm font-bold mt-1">{homeTeam?.name || 'TBD'}</p>
                          </div>
                          <div className="px-4 text-center">
                            {match.status === 'Completed' ? (
                          ...
                          <div className="text-center flex-1">
                            <MediaRenderer ''} className="w-10 h-10 mx-auto rounded-full" url="{awayTeam?.logo" ||/>
                            <p className="text-sm font-bold mt-1">{awayTeam?.name || 'TBD'}</p>
                          </div>
                        </div>
                ...
                <div className="mt-10 border-t border-slate-800 pt-8">
                  <h3 className="text-xl font-extrabold text-white flex items-center space-x-2 border-b border-slate-800 pb-4 mb-6">
                    <Trophy className="w-5 h-5 text-amber-400"/><span>Knockout Stages & Playoffs</span>
                  </h3>

                  <div className="space-y-8">
                    {/* Quarter Finals Play-Offs */}
                    <div>
                      <h4 className="text-lg font-bold text-emerald-400 mb-4 flex items-center space-x-2">
                        <Shield className="w-4 h-4"/><span>Quarter Finals Play-Offs</span>
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[1, 2, 3, 4].map(i => (
                          <div key={`qfp-${i}`} className="bg-slate-950 rounded-xl p-4 border border-slate-800 flex justify-between items-center">
                    ...
                    {/* Quarter Finals Knockouts */}
                    <div>
                      <h4 className="text-lg font-bold text-emerald-400 mb-4 flex items-center space-x-2">
                        <Shield className="w-4 h-4"/><span>Quarter Finals Knockouts</span>
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[1, 2, 3, 4].map(i => (
                          <div key={`qfk-${i}`} className="bg-slate-950 rounded-xl p-4 border border-slate-800 flex justify-between items-center">
                            <span className="text-sm font-bold text-slate-400">TBD</span>
                            <span className="text-xs font-black text-slate-600 bg-slate-900 px-2 py-1 rounded">VS</span>
                            <span className="text-sm font-bold text-slate-400">TBD</span>
                          </div>
                    ...
                    {/* Semi-Finals */}
                    <div>
                      <h4 className="text-lg font-bold text-amber-400 mb-4 flex items-center space-x-2">
                        <Flame className="w-4 h-4"/><span>Semi-Finals</span>
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[1, 2].map(i => (
                          <div key={`sf-${i}`} className="bg-slate-950 rounded-xl p-4 border border-amber-900/30 flex justify-between items-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <h4 className="text-lg font-bold text-slate-300 mb-4">Position 3 & 4</h4>
                        <div className="bg-slate-950 rounded-xl p-4 border border-slate-700 flex justify-between items-center">
                          <span className="text-sm font-bold text-slate-300">TBD</span>
                          <span className="text-xs font-black text-slate-600 bg-slate-900 px-2 py-1 rounded">VS</span>
                          <span className="text-sm font-bold text-slate-300">TBD</span>
                        </div>
                      ...
                      <div>
                        <h4 className="text-lg font-bold text-amber-400 mb-4 flex items-center space-x-2">
                          <Trophy className="w-4 h-4"/><span>Grand Final</span>
                        </h4>
                        <div className="bg-slate-950 rounded-xl p-5 border-2 border-amber-500/50 flex justify-between items-center shadow-[0_0_15px_rgba(245,158,11,0.2)]">
                          <span className="text-base font-bold text-white">TBD</span>
                          <span className="text-sm font-black text-amber-400 bg-slate-900 px-3 py-1 rounded border border-amber-900/50">VS</span>
                          <span className="text-base font-bold text-white">TBD</span>
                        </div>
                      </div>
                    </div>
                      ...
                      <tbody className="divide-y divide-slate-800/60">
                        {teams.map((team) => {
                          ...
                          return (
                            <tr key={team.id} className="hover:bg-slate-800/40">
                              <td className="py-3 px-4 font-bold text-white flex items-center space-x-2">
                                <MediaRenderer className="w-6 h-6 rounded-full" url="{team.logo}"/>
                                <span>{team.name}</span>
                              </td>
                              <td className="py-3 px-4 text-emerald-400 font-semibold">
                                {official ? official.officialName : <span className="text-slate-500 italic">Not Assigned</span>}
              ...
              <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 text-center relative overflow-hidden group">
                    <h4 className="text-emerald-400 font-black uppercase tracking-wider mb-3">Player of the Day</h4>
                    <div className="relative w-full h-48 rounded-lg overflow-hidden mb-3 border border-slate-800 bg-slate-900 flex justify-center items-center">
                       <MediaRenderer className="w-full h-full object-contain" url="{potd.mediaUrl}"/>
                    </div>
                    <p className="text-xl font-bold text-white">{potd.name}</p>
                    <p className="text-sm text-slate-400">{potd.team}</p>
                    {isAdmin && (
                  ...
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 text-center relative overflow-hidden group">
                    <h4 className="text-amber-400 font-black uppercase tracking-wider mb-3">Player of the Match</h4>
                    <div className="relative w-full h-48 rounded-lg overflow-hidden mb-3 border border-slate-800 bg-slate-900 flex justify-center items-center">
                       <MediaRenderer className="w-full h-full object-contain" url="{potm.mediaUrl}"/>
                    </div>
                    <p className="text-xl font-bold text-white">{potm.name}</p>
                    <p className="text-sm text-slate-400">{potm.match}</p>
                    {isAdmin && (
